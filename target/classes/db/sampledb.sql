-- 메모리 테이블 "SAMPLE" 생성 (데이터는 디스크가 아니라 메모리에 저장, 디스크보다 읽기/쓰기 빠르게 처리)
CREATE MEMORY TABLE SAMPLE (
	ID VARCHAR(16) NOT NULL PRIMARY KEY, -- 카테고리ID
...
-- 현재 세션에서 PUBLIC 스키마를 기본 스키마로 설정
SET SCHEMA PUBLIC
...
-- 다음 게시물의 순서를 가장 최근 게시물 다음으로 지정 114 -> 115
-- 미지정 시 1번째 게시물로 지정되는 문제 발생
